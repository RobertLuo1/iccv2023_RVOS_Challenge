...
class UNINEXT_VID(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained("/mnt/data_16TB/lzy23/pretrained/bert-base-uncased")
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            if task in ["detection", "grounding"]:
                ...
                video_dict = {}
                mask_dict = {}
                mask_dict[batched_inputs[0]["video"]] = {}
                mask_logits = []
                class_info = []
                ...
                for frame_idx in range(video_len):
                    ...
                    if dataset_name == "bdd_track":
                        ...
                        final_mask, (mask_logit, cl_info) = self.inference_rvos(output, positive_map_label_to_token, num_classes, images.image_sizes, (height, width))
                        video_name, exp_id = batched_inputs[0]["video"], batched_inputs[0]["exp_id"]
                        # save binary image
                        class_info.append(cl_info)
                ...
                if dataset_name == "bdd_track":
                    ...
                    mask_dict[batched_inputs[0]["video"]][exp_id] = {
                        'mask_logits': mask_logits.cpu(), #[t h w]
                        'class_info': class_info # list [t]
                    }
                    return mask_dict
                    # pass
                else:
    ...
    def inference_rvos(self, outputs, positive_map_label_to_token, num_classes, image_sizes, ori_size):
        ...
        for _, (logits, output_mask, output_boxes, output_embed, image_size, output_iou) in enumerate(zip(
            ...
            track_masks = track_masks[:, :, :image_size[0],:image_size[1]] # crop the padding area
            track_masks = F.interpolate(track_masks, size=(ori_size[0], ori_size[1]), mode='nearest') # (1, 1, H, W)
            track_masks_logit = track_masks[0][0] # (H W) logit
            track_masks = (track_masks[:, 0] > 0.5).cpu().numpy() # (N, H, W)
        del outputs
        return track_masks[0], (track_masks_logit, topk_values.item()) #[H W]
