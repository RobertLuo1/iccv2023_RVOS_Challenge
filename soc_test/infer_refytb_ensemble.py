...
def main(config):
    ...
    ensemble_save_path = os.path.join(save_dir, "test", "model_pth")
    # output_dir_path = os.path.join(save_dir_path, "Annotations")
    os.makedirs(ensemble_save_path, exist_ok=True)
    config.ensemble_save_path = ensemble_save_path
    ...
    # sub_processor(config, test_data, img_folder, video_list)
    sub_processor(config, test_data, img_folder, video_list)
    end_time = time.time()
...
def sub_processor(args, data, img_folder, video_list):
    ...
    for video in tqdm(video_list):
        ...
        for i in range(num_expressions):
            ...
            tmp_info = {
                'mask_logits': pred_masks.squeeze(0).cpu(), #[t h w]
                'class_info': class_info # list [t]
            }
            # import pdb; pdb.set_trace()
            tmp_dict[video][exp_id].update(tmp_info)
    # save tmp results
    save_path = os.path.join(args.ensemble_save_path, args.version)
    torch.save(tmp_dict, save_path)


if __name__ == '__main__':
    parser = argparse.ArgumentParser('Ref-Youtube inference script')
