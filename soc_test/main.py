...
def run(process_id, args):
    ...
    if config.running_mode == 'train':
        ...
        if config.use_ema:
            model_state_dict = model_state_dict["ema_model_state_dict"]
            from torch.nn.parallel import DistributedDataParallel as DDP
            trainer.model_ema.load_state_dict(model_state_dict, strict=True)
            trainer.ema_evaluate()
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--use_ema", action="store_true", help="enable tracking Exponential Moving Average of model parameters"
    )
    parser.add_argument(
        "--model_ema_steps",
        type=int,
    ...
    parser.add_argument("--model_ema_start_epoch", type=int, default=0)
    parser.add_argument("--use_cycle_lr", action="store_true", default=False)
    parser.add_argument("--base_lr", type=float, default=0, nargs="+")
