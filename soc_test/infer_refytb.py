...
def main(config):
    ...
    # if split == 'test':
        # split = "valid"
    # save path
    save_dir = '/mnt/data_16TB/lzy23'
    save_dir_path = os.path.join(save_dir, "test", "soc")
    ...
    valid_test_videos = set(valid_data.keys())
    ...
    global result_dict
    global key_frame_dict
    result_dict = mp.Manager().dict()
    key_frame_dict = mp.Manager().dict()
    ...
    json_save_path = os.path.join(save_dir_path, 'key_frame.json')
    key_frames_ids = {}
    for key, key_frame_info in key_frame_dict.items():
        for video, info in key_frame_info.items():
            key_frames_ids[video] = info
    with open(json_save_path, 'w') as f:
        json.dump(key_frames_ids, f)
...
def sub_processor(lock, pid, args, data, save_path_prefix, save_visualize_path_prefix, img_folder, video_list):
    ...
    for video in video_list:
        ...
        for i in range(num_expressions):
            ...
            pred_logits = outputs["pred_is_referred"][:, 0, ...] # [t, q, k]
            pred_masks = outputs["pred_masks"][:, 0, ...]   # [t, q, h, w] 
            pred_boxes = outputs["pred_boxes"][:, 0, ...]  
            # pred_vlogits = outputs["pred_logit"]
            # text_sentence_feature = outputs["text_sentence_feature"]
            # text_sentence_feature = text_sentence_feature.unsqueeze(1) #[B 1 C]
            # query_text_sim = pred_vlogits @ text_sentence_feature.transpose(1, 2) #[B Nq 1]
            # query_text_sim = query_text_sim.squeeze(-1).softmax(dim=-1).unsqueeze(dim=-1) #[B Nq]
            # pred_scores_1 = query_text_sim[0, ...]#[q, 1]
            ...
            pred_masks = (pred_masks > 0.5).squeeze(0).detach().cpu().numpy()
    ...
    key_frame_dict[str(pid)] = key_frames_id
    with lock:
        progress.close()
