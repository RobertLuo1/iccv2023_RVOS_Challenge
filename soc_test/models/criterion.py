...
class SetCriterion(nn.Module):
    ...
    def __init__(self, matcher, weight_dict, eos_coef, text_refer, num_classes):
        ...
        self.use_text_refer_loss = text_refer
        ...
        if weight_dict['loss_label'] > 0:
            losses_to_compute.append('loss_label')
    ...
    def compute_criterion(self, outputs, targets, losses_to_compute):
        ...
        indices = self.matcher(outputs, targets, self.use_text_refer_loss) #[(tensor, tensor)] batchsize
    ...
    def loss_is_referred(self, outputs, targets, indices, **kwargs):
        ...
        bs = outputs['pred_is_referred'].shape[0] #[t*b, n, 2]
        pred_is_referred = outputs['pred_is_referred'].log_softmax(dim=-1)  # note that log-softmax is used here
        target_is_referred = torch.zeros_like(pred_is_referred) #[t*b, n, 2]
        ...
        loss = -(pred_is_referred * target_is_referred).sum(-1) #[1, 0] * log[0.8, 0.2] = 0
                                                                #[0, 1] * log[0.2, 0.8] = 0
                                                                #[0, 1] * log[0.8, 0.1]
                                                                #[1] [0.8] = 0
                                                                #[0] [0.7/0.2] = 0 
        ...
        losses = {'loss_is_referred_assist': loss}
        return losses

    def text_refer_loss(self, output, targets, indices, frame_size, **kwargs):
        ...
        #     # keep only the matched query indices of the frames in which the referred object is visible:
        #     visible_query_referred_indices = query_referred_indices[ref_inst_visible_frame_indices]
        #     gt[ref_inst_visible_frame_indices, visible_query_referred_indices] = torch.tensor([1.0]).to(gt)
        # else:  # assume that the referred object is visible in every frame:
        gt[torch.arange(bs), query_referred_indices] = torch.tensor([1.0], device=device, dtype=gt.dtype)
        ## use focal loss
        # gamma = 2
        # alpha = 0.25
        # inputs = query_text_sim
        # loss = F.cross_entropy(inputs, gt)
        # p_t = inputs * gt + (1 - inputs) * (1 - gt)
    ...
    def loss_label(self, outputs, targets, indices, num_masks, **kwargs):
        device = outputs['pred_is_referred'].device
        frames_size, bs, nq, k = outputs['pred_is_referred'].shape #[t b, nq, K] K=1 when a2d
        pred_label = rearrange(outputs["pred_is_referred"], 't b nq k -> b (t nq) k')
        BT = len(targets)
        B = BT // frames_size
        batch_targets = []
        for i in range(B):
    ...
    def get_loss(self, loss, outputs, targets, indices, **kwargs):
        if self.use_text_refer_loss:
            loss_map = {
                'masks': self.loss_masks,
                'is_referred': self.text_refer_loss,
