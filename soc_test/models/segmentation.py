...
class TemporalHead(nn.Module):
    """
    Using FPN-like for computing the Final Segmentation mask. 
    """
    def __init__(self, context_dim, fpn_dims, mask_kernels_dim=8) -> None:
        super().__init__()
        inter_dims = [context_dim, context_dim // 2, context_dim // 4, context_dim // 8, context_dim // 16]
        self.lay1 = Conv21(context_dim, inter_dims[0])
        self.gn1 = torch.nn.GroupNorm(8, inter_dims[0])
        self.lay2 = Conv21(inter_dims[0], inter_dims[1])
        self.gn2 = torch.nn.GroupNorm(8, inter_dims[1])
        self.lay3 = Conv21(inter_dims[1], inter_dims[2])
        self.gn3 = torch.nn.GroupNorm(8, inter_dims[2])
        self.lay4 = Conv21(inter_dims[2], inter_dims[3])
        self.gn4 = torch.nn.GroupNorm(8, inter_dims[3])
        self.out_layer = Conv21(inter_dims[3], mask_kernels_dim)
    ...
    def forward(self, x: Tensor, layer_features: List[Tensor]):
        ...
        x = rearrange(x, '(t b) c h w -> b c t h w', t=t, b=b)
        x = self.lay3(x)
        x = self.gn3(x)
        x = F.relu(x)
...
class Conv21(nn.Module):
    """
    Replace the 3D convolution for (2+1)Conv
    """
    def __init__(self, inchs, outchs, rate =None ,use_dilation=False) -> None:
        super().__init__()
        if not use_dilation:
            self.spatial_conv = nn.Conv3d(inchs, outchs, kernel_size=(1, 3, 3), padding=(0, 1, 1))
            self.temporal_conv = nn.Conv3d(outchs, outchs, kernel_size=(3, 1, 1), padding=(1, 0, 0))
        else:
            self.spatial_conv = nn.Conv3d(inchs, outchs, kernel_size=(1, 3, 3), padding=(0, rate, rate), dilation=(0, rate, rate))
            self.temporal_conv = nn.Conv3d(inchs, outchs, kernel_size=(3, 1, 1), padding=(rate, 0, 0), dilation=(rate, 0, 0))
