...
class A2DSentencesPostProcess(nn.Module):
    ...
    def forward(self, outputs, resized_padded_sample_size, resized_sample_sizes, orig_sample_sizes):
        ...
        """
        pred_is_referred = outputs['pred_is_referred'] #[t b nq 1]
        pred_is_referred = pred_is_referred.flatten(0, 1) #t*b nq, 1
        
        # prob = F.softmax(pred_is_referred, dim=-1) #[t*B，Query, 2]
        scores = pred_is_referred[..., 0].sigmoid()
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        """
        # coco, num_frames=1
        out_logits = rearrange(outputs["pred_is_referred"], 't b nq k -> b (t nq) k')
...
class PostProcessSegm(nn.Module):
    ...
    def forward(self, results, outputs, orig_target_sizes, max_target_sizes):
        ...
        # out_logits = outputs["pred_is_referred"].flatten(1, 2) #[t b nq K]
...
class COCOPostProcess(nn.Module):
    ...
    def forward(self, outputs, resized_padded_sample_size, resized_sample_sizes, orig_sample_sizes):
        ...
        # prob = F.softmax(pred_is_referred, dim=-1) #[B*Query, 2]
        # scores = prob[..., 0]
...
class ReferYoutubeVOSPostProcess(nn.Module):
    ...
    def forward(self, outputs, videos_metadata, samples_shape_with_padding):
        ...
        """
        pred_is_referred = outputs['pred_is_referred'].sigmoid()  #[t, b, nq, k]
        prob_is_referred = pred_is_referred.mean(0)  #[b, nq, k]
...
#             orig_sample_sizes: original size of the samples (no augmentations or padding)
#         """
#         # pred_is_referred = outputs['pred_is_referred']    #[b, nq, 2]
#         # prob = F.softmax(pred_is_referred, dim=-1) #[t, B*Query, 2]
#         # scores = prob[..., 0]    # [2, nq]
#         pred_masks = outputs['pred_masks']  #[t*b n h w]
#         pred_masks = F.interpolate(pred_masks, size=resized_padded_sample_size, mode="bilinear", align_corners=False)
#         pred_masks = (pred_masks.sigmoid() > 0.5)
#         processed_pred_masks, rle_masks = [], []
...
#         '''
#         scores: [t*b,nq]
#         processed_pred_masks: a list of [n, 1, h, w] with length of t*b
#         rle_masks : the same size with processed_pred_masks
#         t = 1
