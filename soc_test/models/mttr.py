...
from models.vla import VisualLanguageALignment, VisionLanguageFusionModule, LanguageVisionModule
...
class MTTR(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.vita = VITA(config.VITA)

        # self.vla = VisualLanguageALignment(d_model, d_model)
        self.vlf = VisionLanguageFusionModule(d_model=d_model, nhead=8)
        self.lvf = VisionLanguageFusionModule(d_model=d_model, nhead=8)
        ...
        self.text_refer_loss = config.text_refer_loss
        # if self.text_refer_loss:
    ...
    def forward(self, samples: NestedTensor, valid_indices, text_queries, targets):
        ...
        if valid_indices is not None:
            for layer_out in backbone_out:
                # layer_out.tensors = layer_out.tensors[valid_indices, torch.arange(valid_indices.size()[0]), ...].unsqueeze(0)
                # layer_out.mask = layer_out.mask[valid_indices, torch.arange(valid_indices.size()[0]), ...].unsqueeze(0)
        ...
        if self.text_refer_loss:
            # text feature for text_refer_loss
            text_features = []
            tempt = []
        ...
        vita_hs = self.vita(hs, text_sentence_feature) # vita_hs [L B NQ C]

        # residual 
        frame = hs.shape[1]
        hs_vita = repeat(vita_hs, 'l b n c -> l t b n c', t=frame)
        ...
        outputs_classes = []
        outputs_coords = []
        hs_vita = rearrange(hs_vita, 'l t b n c -> l (b t) n c')
        for lvl in range(hs_vita.shape[0]):
            if lvl == 0:
                reference = init_reference
            else:
                reference = inter_references[lvl - 1]
            reference = inverse_sigmoid(reference)
            outputs_class = self.class_embed[lvl](hs_vita[lvl])
            tmp = self.bbox_embed[lvl](hs_vita[lvl])
        ...
        ####
        # hs_vita = rearrange(hs_vita, 'l (b t) n c -> l t b n c', b=B, t=T)
        ...
        # # bbox_attention = self.bbox_attention(hs_vita[-1], memory[-1])
        ...
        # instance_kernels = self.instance_kernels_head(hs_vita)  # [L, T, B, N, C]
        # # instance_kernels = self.instance_kernels_head(vita_hs) #[L B N d] d=8
        ...
        outputs_seg_masks = []
        for lvl in range(hs_vita.shape[0]):
            dynamic_mask_head_params = self.controller(hs_vita[lvl])   # [batch_size*time, num_queries_per_frame, num_params]
        ...
        # if self.text_refer_loss:
        #     outputs_is_referred = self.is_referred_head(hs_vita)  # [L,T, B, N, 2]
            ...
            # pdb.set_trace()
        if self.text_refer_loss:    
            for pm, plg, pir, pb in zip(output_masks, vita_hs, outputs_classes, outputs_coord):
...
def build(args):
    ...
    weight_dict = {'loss_is_referred': args.is_referred_loss_coef,
                   ...
                   'loss_label': args.is_referred_loss_coef_assist,
    ...
    criterion = SetCriterion(matcher=matcher, weight_dict=weight_dict, eos_coef=args.eos_coef, text_refer=args.text_refer_loss, num_classes=args.num_classes)
    criterion.to(device)
