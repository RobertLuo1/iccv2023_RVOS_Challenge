...
def main(args):
    ...
    # output_dir_path = os.path.join(save_dir, 'runs', args.dataset_file, args.version)
    output_dir_path = os.path.join(save_dir, "test", "referformer_ft", "Annotations")
    ...
    for i in range(thread_num):
        ...
        p = mp.Process(target=sub_processor, args=(lock, i, args, test_data, 
...
def sub_processor(lock, pid, args, data, save_path_prefix, save_visualize_path_prefix, img_folder, video_list):
    ...
    if args.resume:
        ...
        print('Please specify the checkpoint for inference.')
